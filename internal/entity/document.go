...
type Document struct {
	...
	ProcessingTaskID *uuid.UUID `json:"processing_task_id"` // 关联的处理任务 ID
...
type DocumentChunk struct {
	ID         uuid.UUID       `json:"id"`          // 块的唯一 ID
	DocumentID uuid.UUID       `json:"document_id"` // 所属文档的 ID
...
func NewDocumentChunk(documentID uuid.UUID, userID string, chunkIndex int, content string, embedding pgvector.Vector, metadata map[string]any) *DocumentChunk {
